            ...
            tipo : INTEGER {$$.obj = $1.obj;
                ...
                lista_variables :IDENTIFICADOR  {$$.obj=new ArrayList<Token>();
                  ...
                  asig_for:  IDENTIFICADOR operador_asignacion expresion ';' {System.out.println("isssssss show time");
                    ...
                    cond_for: expresion comparador expresion ';' {System.out.println("=== COND FOR ==== ");
                                                                  System.out.println("terceto size "+tercetos.size());
                                                                  tercetos.add(new TercetoLabel((Integer)tercetos.size()+2,(Integer)tercetos.size()+1));
                ...
                sentencia_ejecutable : inicio_IF {estructuras.add(numberLine.pop()+". sentencia ejecutable if\n");}
                          ...
                          // | FOR  '(' IDENTIFICADOR operador_asignacion expresion ';' expresion comparador expresion ';' variable DECREMENTO ')' error  {UI2.addText(UI2.txtDebug,"Linea: "+numberLine.peek()+". ERROR EN SENTENCIA FOR  "+"\n"); estructuras.add(numberLine.peek()+". sentencia ejecutable for\n"); errores.add("Linea: "+numberLine.pop()+"ERROR EN SENTENCIA FOR");}
                                     ...
                                     | PRINT '(' CADENA ')' ';' {System.out.println("===CADENA "+ (Element)$3.obj+" palabra reservada "+(Token)$1.obj);
      ...
      | termino '/' factor{
        ...
        constante : CTEINTEGER {$$.obj = lexAn.getSymbolTable().getToken(((Token)$1.obj).getLexema()); //ANDA
          ...
          variable : IDENTIFICADOR {
            ...
            } else { if ( lexAn.getSymbolTable().containsSymbol("var@"+((Token)$1.obj).getLexema()) ){
							$$.obj = lexAn.getSymbolTable().getToken("var@"+((Token)$1.obj).getLexema());
						} else {
            ...
            if ( !((Element)$3.obj).getTypeVariable().equals("integer") || !((Element)$6.obj).getTypeVariable().equals("integer") ){
              ...
              errores.add("Linea: "+lexAn.getLineNumber()+"Variable matriz con limites de tipo erroneos");
				...
				if (lexAn.getSymbolTable().containsSymbol("mat@"+((Token)$1.obj).getLexema())){
							identificador = lexAn.getSymbolTable().getToken("mat@"+((Token)$1.obj).getLexema());
						} else {
								identificador = lexAn.getSymbolTable().getToken(((Token)$1.obj).getLexema());
							}
              String typeVariable = lexAn.getSymbolTable().getToken(identificador.getLexema()).getTypeVariable();
              System.out.println("elementos enviados a make matrix fila"+((Element)$3.obj)+"  columna"+((Element)val_peek(1).obj));
              makeMatrix(identificador,((Element)$3.obj),((Element)val_peek(1).obj));
              ...
              $$.obj = tercetos.get(tercetos.size()-1);
              ((Element)$$.obj).setTypeVariable(typeVariable);
              ((Element)$$.obj).setUse("mat");
